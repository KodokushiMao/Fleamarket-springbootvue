      ...
      <el-container style="height: 700px; border: 1px solid #eee">
        ...
        <el-main style="height:0;flex-grow:1;">
            ...
            @row-click="openDetails" border>
                <el-table-column prop="account" label="用户id" width="140">
                </el-table-column>
                ...
                <el-table-column prop="bankcard" label="银行账户" width="230">
                </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getData () { // 后台获取数据
      ...
      formData.append('account', this.userid)
      formData.append('password', this.userword)
      let config = {
        headers: {
    ...
    getList () { // 根据id搜索结果集
      this.tableData = this.table.filter((item, index) =>
        item.account.includes(this.search)
      )
      this.length = this.tableData.length
    },
    clear () { // 搜索转全部
    ...
    sure () { // 确认允许登录，数据库删除信息，同时删除表格相应数据
      ...
      this.$axios.post('http://localhost:8088/deleteUser', formData, config)
        .then(res => {
          this.$notify({
            title: '成功',
            message: '操作成功',
      ...
      formData1.append('password', this.column.password)
      ...
      formData1.append('bankcard', this.column.bankcard)
      let config1 = {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
