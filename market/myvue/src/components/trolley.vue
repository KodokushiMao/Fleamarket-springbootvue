      ...
      <el-container style="height: 700px; border: 1px solid #eee">
        ...
        <el-main style="height:0;flex-grow:1;">
            ...
            @row-click="openDetails" border>
                ...
                <el-table-column prop="details" label="商品简介" width="150">
                </el-table-column>
                <el-table-column prop="oan" label="新旧程度/成" width="80">
                </el-table-column>
                <el-table-column prop="type" label="商品类别" width="80" >
                </el-table-column>
                <el-table-column prop="model" label="商品尺寸" width="100" >
                </el-table-column>
                <el-table-column prop="counteroffer" label="是否议价" width="60" >
                </el-table-column>
                <el-table-column prop="num" label="购买数量" width="210" >
                    <template slot-scope="scope">
                    <el-input-number v-model="scope.row.num" @change="(currentValue, oldValue) => handleChange(currentValue, oldValue, scope.$index)"
                    :min="1"  label="描述文字"></el-input-number>
                    </template>
                </el-table-column>
                <el-table-column prop="stock" label="商品库存" width="80" >
                </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleChange (currentValue, oldValue, index) {
      ...
      formData.append('num', this.tableData[(this.currentPage - 1) * this.pageSize + index].count)
      let config = {
        headers: {
      ...
      if (this.multipleSelection.length === 0) {
        ...
        if (c > 0) {
          ...
          for (var i = 0; i < this.multipleSelection.length; i++) {
            this.money += this.multipleSelection[i].num * this.multipleSelection[i].price
          }
    ...
    onSubmit () {
      if (this.money > sessionStorage.getItem('money')) {
        ...
        formData.append('username', this.form.uname)
        formData.append('address', this.form.address)
        formData.append('remainder', parseFloat(this.money).toFixed(2))
        let config = {
          headers: {
            'Content-Type': 'multipart/form-data'
          }
