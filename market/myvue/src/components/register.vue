...
<script>
import security from './verificationcode'
export default {
  data () {
    ...
    submitForm (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          formData.append('account', this.ruleForm.id)
          let config = {
            headers: {
          ...
          this.$axios.post('http://localhost:8088/getZhuceUser', formData, config)
            .then(res => {
              if (res.data.length > 0) {
                ...
                formData1.append('account', this.ruleForm.id)
                formData1.append('password', this.ruleForm.pass)
                ...
                formData1.append('bankcard', this.ruleForm.acount)
                let config1 = {
                  headers: {
                    'Content-Type': 'multipart/form-data'
                  }
