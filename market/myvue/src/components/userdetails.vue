    ...
    getInfo () {
      ...
      this.$axios.post('http://localhost:8088/personal', formData, config
        ...
        if (Response.data.length === 1) {
          this.ruleForm.id = Response.data[0].account
          ...
          this.ruleForm.account = Response.data[0].bankcard
          this.ruleForm.status = Response.data[0].identity
        } else {
          this.$router.push('/login')
        }
      })
    ...
    submitForm (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          formData.append('bankcard', this.ruleForm.account)
          formData.append('account', sessionStorage.getItem('userid'))
          formData.append('password', sessionStorage.getItem('userword'))
          let config = {
            headers: {
