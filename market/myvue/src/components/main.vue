<template>
  <div class="all" id="box" >
    ...
    <ul v-for="item in table" :key="item">
      ...
      <li>
        <p @click="info(item)" style="text-decoration:underline">{{item.details}}</p>
      </li>
      <li>
        <a @click="shop(item)" style="text-decoration:underline">{{item.account}}</a>的店铺
      </li>
      </div>
    </ul>
      </div>
...
export default {
  ...
  methods: {
    ...
    shop (item) {
      sessionStorage.setItem('shopid', item.account)
